...
export default function UnitServicesTab({ unitId, readOnly = false, onCountChange, onAttachmentsChanged }: Props) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (isCustomService && !formValue.name?.trim()) {
      toast.showSuccess('Vyplňte název vlastní služby.')
      return
    }
    ...
    if (isCopyMode) {
      ...
      if (hasDuplicateServiceName(services, selectedId, formValue.name)) {
        toast.showSuccess('Služba se stejným názvem už v jednotce existuje. Upravte název.')
        return
      }
    }
  ...
  }, [copySource?.name, copyValidFrom, detailMode, formValue, isCopyMode, isCustomService, reloadServices, selectedId, selectService, services.length, toast, unitId])
