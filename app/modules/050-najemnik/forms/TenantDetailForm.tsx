...
// FILE: app/modules/030-pronajimatel/forms/TenantDetailForm.tsx
...
export type TenantDetailFormRef = {
...
const TenantDetailForm = React.forwardRef<TenantDetailFormRef, TenantDetailFormProps>(
  ...
  const initial = useMemo<TenantFormValue>(
    () => ({
      ...
      lastName: safe(tenant.lastName),
      birthDate: safe(tenant.birthDate),
      personalIdNumber: safe(tenant.personalIdNumber),
      idDocType: safe(tenant.idDocType),
      idDocNumber: safe(tenant.idDocNumber),

      // Adresa
      street: safe(tenant.street),
      city: safe(tenant.city),
      zip: safe(tenant.zip),
      houseNumber: safe(tenant.houseNumber),
      country: safe(tenant.country || 'CZ'),

      // Základní údaje
      displayName: safe(tenant.displayName),
      email: safe(tenant.email),
      phone: safe(tenant.phone),

      // Company fields
      companyName: safe(tenant.companyName),
      ic: safe(tenant.ic),
      dic: safe(tenant.dic),
      icValid: !!tenant.icValid,
      dicValid: !!tenant.dicValid,
      delegateIds: Array.isArray(tenant.delegateIds) ? tenant.delegateIds : [],

      // Role flags
      isUser: !!tenant.isUser,
      isTenant: tenant.isTenant !== undefined ? !!tenant.isTenant : true, // Default true protože editujeme pronajimatele
      isTenantDelegate: !!tenant.isTenantDelegate,
      isTenant: !!tenant.isTenant,
      isTenantDelegate: !!tenant.isTenantDelegate,
      isMaintenance: !!tenant.isMaintenance,
      isMaintenanceDelegate: !!tenant.isMaintenanceDelegate,

      // Poznámka
      note: safe(tenant.note),
      isArchived: !!tenant.isArchived,
    }),
    [tenant]
  )

  const [val, setVal] = useState<TenantFormValue>(initial)
  const dirtyRef = useRef(false) // Ref pro okamžitou kontrolu
  const [loadingAres, setLoadingAres] = useState(false)
  const [errors, setErrors] = useState<Record<string, string>>({})
  const toast = useToast()

  // Když se změní tenant, přepiš form
  useEffect(() => {
    console.log('[TenantDetailForm] useEffect triggered, initial changed. dirty:', dirtyRef.current)
    // Pokud je formulář dirty, neresetuj ho - uživatel právě edituje
    if (dirtyRef.current) {
      console.log('[TenantDetailForm] Skipping reset because form is dirty')
      return
    }
    // Vytvořit nový objekt, ne jen referenci
    console.log('[TenantDetailForm] Resetting form to initial values:', initial)
  ...
  const update = (patch: Partial<TenantFormValue>) => {
    console.log('[TenantDetailForm] update() called with:', patch)
    setVal((prev) => {
      const next = { ...prev, ...patch }
      console.log('[TenantDetailForm] New form value:', next)

      if (!dirtyRef.current) {
        console.log('[TenantDetailForm] Setting dirty to true')
